...
import settlementmultiplayer.SettlementMultiplayer;
            ...
            if (mob.removed()) {
                ...
                if (attackOwner != null && attackOwner.isPlayer) {
                    ...
                    if (data != null && mob.isHuman) {
                        ...
                        if (!layerData.isPvpFlagged && !mob.getLevel().settlementLayer.doesClientHaveAccess(client) && !SettlementMultiplayer.getSettings().allowOwnedSettlerKillsNoPvP) {
                            return true;
                        } else {
        ...
        static boolean onExit(@Advice.This HumanMob humanMob, @Advice.Argument(0) Mob attacker, @Advice.Argument(1) NetworkClient client, @Advice.Return(readOnly = false) boolean out) {
            if (client != null) {
                ...
                if (!client.pvpEnabled() && humanMob.owner.get() != -1L) {
                    out = false;
                    return out;
                } else if (humanMob.owner.get() != -1L && SettlementMultiplayer.getSettings().requireSettlerOwnerOnlineToKill && team != null && team.streamMembers().noneMatch((l) -> humanMob.getLevel().getClient().getClientByAuth(l) != null)) {
                    out = false;
                    return out;
                }
